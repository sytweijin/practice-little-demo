...
- `refreshAll()` 是删除、移动、确认、回忆等操作后的集中刷新入口，此前调用了 `loadCards / loadLedger / loadRecall / loadGraph`，但遗漏了 `loadProfiles`，导致档案下拉（如「default（16 张）」）的张数不随卡片增删变化。现已补入 `loadProfiles()`。
