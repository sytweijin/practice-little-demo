...
fetch('/api/cards/search?q='+encodeURIComponent(q)).then(function(r){return r.json();}).then(function(d){var cards=d.cards||[];
...
return"<div class='search-result-item' data-id='"+c.id+"'><span class='search-result-scene scene-"+c.scene_type+"'>"+esc(sn)+"</span><span class='search-result-title'>"+esc(c.title)+"</span></div>";}).join('');
...
var b=new Blob([JSON.stringify(d,null,2)],{type:'application/json'}),u=URL.createObjectURL(b),a=document.createElement('a');a.href=u;a.download='presence_backup_'+new Date().toISOString().slice(0,10)+'.json';a.click();URL.revokeObjectURL(u)
}).catch(function(e){alert('导出失败：'+e.message)}).finally(function(){eb.textContent='⬇导出全部卡片（JSON备份）';eb.disabled=false})}
if(ib&&ii){ib.onclick=function(){ii.click()};ii.onchange=function(){var f=ii.files[0];if(!f)return;var r=new FileReader();r.onload=function(e){
try{var d=JSON.parse(e.target.result),cl=d.cards||[];if(!cl.length){alert('文件中没有可导入的卡片');return}
var merge=confirm('智能同步：已存在的卡片将保留更新的版本，回忆进度取较高值。\n点确定开始同步，点取消跳过。')
  if(!merge)return
fetch('/api/sync/import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)}).then(function(r){return r.json()}).then(function(res){alert('同步完成！\n新增：'+res.imported+'张 \xb7 更新：'+res.updated+'张 \xb7 跳过：'+res.skipped+'张');if(typeof loadCards==='function')loadCards();if(typeof loadLedger==='function')loadLedger()}).catch(function(err){alert('导入失败：'+err.message)})}catch(err){alert('文件格式错误，请选择有效的JSON备份文件')}};r.readAsText(f);ii.value=''}}

if(mtb)mtb.onclick=function(){fetch('/api/tags').then(function(r){return r.json()}).then(function(d){var tags=d.tags||[];var body=document.getElementById('modalBody');
...
if(action==='rename'){var nn=prompt('将标签「'+tag+'」重命名为：',tag);if(nn&&nn.trim()&&nn.trim()!==tag){fetch('/api/tags/rename',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({old:tag,new:nn.trim()})}).then(function(r){return r.json()}).then(function(){if(typeof loadCards==='function')loadCards();mtb.click()})}}
if(action==='delete'){if(confirm('确定删除标签「'+tag+'」？\n该标签将从所有卡片中移除。')){fetch('/api/tags/rename',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({old:tag,new:''})}).then(function(r){return r.json()}).then(function(){if(typeof loadCards==='function')loadCards();mtb.click()})}}}})
...
var qmt=document.getElementById('quickModeToggle'),qm=false,ab=document.getElementById('analyzeBtn');
...
if(ab){var ac=ab.onclick;ab.onclick=function(){if(qm){var notes=document.getElementById('notesInput').value;if(selectedFiles.length===0&&!notes.trim()){alert('请上传至少一个素材，或输入文字备注');return}ab.disabled=true;ab.textContent='⏳快速记录中...';var fd=new FormData();fd.append('scene_type',selectedScenario);fd.append('quick_mode','true');fd.append('personalization',document.getElementById('personalizationInput').value||'');fd.append('notes',notes);var sendFiles=selectedFiles.slice();var frameFiles=[];
var vFiles=selectedFiles.filter(function(f){return f.type.startsWith('video/')});
